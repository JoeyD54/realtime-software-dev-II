...
namespace Azul
{
		...
		: GameObjectAnim(pGraphicsObject)
	{
		...
		this->poLocal = new Mat4(Mat4::Special::Identity);
	//	this->index = -1;
		this->poBoneOrientation = new Mat4(Mat4::Special::Identity);
	...
	void GameObjectAnimSkin::privUpdate(AnimTime currentTime)
	{
		...
		Mat4 matParent(Mat4::Special::Identity);
		if(pParentObject)
		{
			...
			T.set(Mat4::Trans::XYZ, this->pBoneResult[index].T);
			S.set(Mat4::Scale::XYZ, this->pBoneResult[index].S);
			Q = this->pBoneResult[index].Q;
		}
		else
		{
			T.set(Mat4::Trans::XYZ, 0, 0, 0);
			S.set(Mat4::Scale::XYZ, 1, 1, 1);
