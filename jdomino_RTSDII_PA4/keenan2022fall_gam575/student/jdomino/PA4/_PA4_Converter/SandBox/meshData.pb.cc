...
uint8_t* meshData_proto::_InternalSerialize(
  ...
  if (_internal_has_pmeshname()) {
    target = stream->WriteBytesMaybeAliased(
        1, this->_internal_pmeshname(), target);
  }
  ...
  if (_internal_has_tricount()) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt32ToArray(3, this->_internal_tricount(), target);
  }

  // optional uint32 vertCount = 4;
  if (_internal_has_vertcount()) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt32ToArray(4, this->_internal_vertcount(), target);
  }
