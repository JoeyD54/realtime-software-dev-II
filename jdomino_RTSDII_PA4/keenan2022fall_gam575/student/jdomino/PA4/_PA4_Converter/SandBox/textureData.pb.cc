...
uint8_t* textureData_proto::_InternalSerialize(
  ...
  if (_internal_has_component()) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt32ToArray(8, this->_internal_component(), target);
  }

  // optional uint32 bits = 9;
  if (_internal_has_bits()) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt32ToArray(9, this->_internal_bits(), target);
  }

  // optional uint32 as_is = 10;
  if (_internal_has_as_is()) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt32ToArray(10, this->_internal_as_is(), target);
  }
  ...
  if (_internal_has_filenamesize()) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt32ToArray(12, this->_internal_filenamesize(), target);
  }

  // optional bytes fileNameData = 13;
  if (_internal_has_filenamedata()) {
    target = stream->WriteBytesMaybeAliased(
        13, this->_internal_filenamedata(), target);
  }

  // optional uint32 dataSize = 14;
  if (_internal_has_datasize()) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt32ToArray(14, this->_internal_datasize(), target);
  }

  // optional bytes poData = 15;
  if (_internal_has_podata()) {
    target = stream->WriteBytesMaybeAliased(
        15, this->_internal_podata(), target);
  }
